...
- [[DPI/tspu-dns-nsdi-dnat-august-2026|Перехват DNS на ТСПУ (август 2026)]] — когда сменить резолвер уже недостаточно: открытый DNS по UDP заворачивают на серверы НСДИ, а DoH-домены режут по SNI — именно тот случай, который лечится хостлистом
...
- [[cloudflare-quick-tunnel|Quick Tunnel от Cloudflare]] — среди прочего: в cloudflared версии 2026.2.0 удалили режим `proxy-dns`, которым многие поднимали локальный DoH-клиент
