...
Через эту рамку события 2026 года складываются в последовательность экспериментов над разными слоями обхода. Таблица ниже — живая хроника: сюда добавляются новые инциденты по мере того, как их разбирают в отдельных заметках, поэтому список со временем растёт. Последнее пополнение — 29 августа 2026.

| Инцидент | Слой | Что тестировали (по версии сообщества) |
|---|---|---|
| [[DPI/nsdi-domain-removal-2026\|Февраль–март 2026: домены убрали из НСДИ]] | DNS / реестр | Блокировка не разрывом соединения, а отсутствием записи: у YouTube и WhatsApp просто нет адреса в государственной базе. К марту без записей — 368 доменов из 2821 по данным «Монитора DNS» |
| [[DPI/tspu-3xui-scmininterval-trap\|11 июня 2026: ловушка обновления 3x-ui]] | Поведение / тайминги | Реакция на частоту и интервалы отправки данных: один параметр в конфиге прокси делает соединение узнаваемым |
...
| [[DPI/tspu-whitelist-cloudflare-june-2026\|23 июня 2026: слетели белые списки Cloudflare]] | Подсети/хостинг | Ковровые блоки диапазонов облаков с «белым списком» исключений — отсечение VPN-хостинга целыми AS |
| [[DPI/twitch-block-2026\|23 июня 2026: блокировка Twitch]] | Сервис / поддомены | Точечное отсечение видеопотока при живом сайте — фильтрация по конкретным поддоменам вместо блокировки домена целиком |
...
| [[Zapret/flowseal-zapret-discord-youtube-block-july-2026\|10 июля 2026: блокировка репозитория Flowseal на GitHub]] | Распространение инструментов | Давление не на трафик, а на канал доставки самих средств обхода |
| [[DPI/mincifry-whitelist-vpn-hosting-august-2026\|Август 2026: «замаскированные» VPN и белый список у хостеров]] | Хостинг / регулирование | Перенос фильтрации на владельцев инфраструктуры: не блокировать протокол, а не дать его разместить |
| [[DPI/tspu-dns-nsdi-dnat-august-2026\|21–28 августа 2026: перехват открытого DNS и блокировка DoH по SNI]] | DNS, оба транспорта | Клещи: шифрованный DNS рвут по имени сервера, открытый заворачивают на резолверы НСДИ с подменой ответа. Фильтр сдвигается на шаг раньше — до установки соединения |

Хронология показывает и смену приоритетов: первая половина года — эксперименты с распознаванием протоколов и ковровыми блоками подсетей, вторая — методичный перенос фильтра на слой имён. Разрешение имени дешевле анализа трафика: нерезолвящийся домен не порождает соединения, которое надо разбирать. К августу этот подход закрыли с обеих сторон — и по открытому DNS, и по шифрованному.
...
- [[DPI/tspu-dns-nsdi-dnat-august-2026|Блокировка (перехват) DNS Google и Cloudflare через ТСПУ]] — августовская итерация плана: фильтр сдвинули на слой разрешения имён.
- [[DPI/nsdi-domain-removal-2026|НСДИ как рубильник]] — предыстория того же слоя: домены убрали из государственной базы ещё в феврале 2026.
...
- [[DPI/tspu-whitelist-cloudflare-june-2026|Инцидент 23 июня 2026: белые списки Cloudflare]] — ковровые блоки подсетей как один из инструментов плана.
...
- [[DPI/isp-licensing-27-regions-september-2026|27 регионов без местных провайдеров]] — сентябрьские параметры реформы и расчёт её последствий по регионам.
