...
Если нужно вернуть один-два конкретных сайта прямо сейчас, проще всего обойтись без DNS вообще. Файл `hosts` (`C:\Windows\System32\drivers\etc\hosts` в Windows, `/etc/hosts` в Linux и macOS) система читает **раньше**, чем идёт к любому резолверу: записанное там имя не резолвится, подменять по дороге нечего. Настоящие адреса берутся из источника, который на вашем соединении отвечает честно, — `dig +tcp +short www.youtube.com @8.8.8.8`, тот же запрос через DoT или из-под туннеля.

Приём дешёвый, но узкий: масок вроде `*.googlevideo.com` файл не понимает, адреса крупных сервисов меняются и со временем протухают, а от блокировки по имени в TLS-приветствии он не спасает вовсе — там нужен [[Zapret2/Zapret2|zapret]]. Про сам файл, его правку и встроенный редактор в Zapret GUI — в заметке [[Zapret/hosts|про hosts и GEO-ограничения]].
