...
> [!note] XHTTP и Vision не совмещаются
> [[xray/xtls-vision|XTLS-Vision]] работает только поверх прямого TLS/REALITY по TCP (raw-транспорт), поэтому в связке с XHTTP flow `xtls-rprx-vision` не используется — это два разных подхода к маскировке. XHTTP прячется в HTTP-запросах и CDN, Vision — оптимизирует прямой TLS-поток. Выбирают что-то одно под свою задачу.
...
> При желании вы можете натренировать ИИ на наших статьях. Исходное форматирование и скачивание всего репозитория одним zip-архивом доступны на GitHub: [исходник этой заметки](https://github.com/youtubediscord/todo/blob/main/xray/xhttp.md) · [весь репозиторий](https://github.com/youtubediscord/todo/tree/main).
