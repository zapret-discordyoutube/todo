...
> [!info] О чём заметка
> Разбор **VLESS Encryption** (*vlessenc, шифрование VLESS, mlkem768x25519plus*) — слоя шифрования, который в сентябре 2025 года появился внутри самого протокола [[xray/vless|VLESS]] и снял его историческую зависимость от внешнего TLS. Здесь: что делает команда `xray vlessenc`, как читается длинная строка параметра, какая под этим криптография, что она защищает, а что нет, и главный практический вопрос — помогает ли она обходить блокировки в России и Китае (короткий ответ: напрямую нет, и автор пишет об этом прямым текстом). Разбор слоёв стека и матрица совместимости «что с чем работает» — в [[xray/vless-stack-map|карте слоёв VLESS-стека]].
...
Срок жизни тикета задаёт сервер; генератор ставит `600s`, а рекомендация автора — около десяти минут. Захардкоженного значения по умолчанию нет.
...
>
> Важная оговорка, которая часто теряется: «пробиваемость» `native` и `xorpub` работает **только когда под шифрованием лежит голый TCP без внешнего слоя безопасности**. Если поверх VLESS Encryption надстроены TLS или REALITY, ядро запрещает splice — под шифрованием уже не сырой сокет.
>
...
> Частый вопрос: раз VLESS Encryption сам шифрует, нужен ли ещё `flow=xtls-rprx-vision`? В сентябре 2025 его задали автору Xray напрямую — в контексте связки с XHTTP, — и ответ был односложным: «рекомендуется включать». Логика в том же PR: XTLS избавляет от повторного шифрования уже зашифрованного потока, то есть без него вы платите за двойную работу. Пустой `flow` вдобавок означает, что набивка внутреннего рукопожатия не применяется вовсе.
...
| Форки sing-box | Да, клиентская часть | Karing (с 1.2.19.2209, 26 мая 2026) |
...
> RPRX сформулировал курс дважды. 28 августа 2025 года: «постепенно ограничивать по умолчанию незашифрованный трафик в публичной сети необходимо — это даёт пользователям гарантию безопасности и защищает от случайной ошибки в конфиге; кто точно понимает, что делает, сможет включить обход сам, а новичок просто не получит незашифрованный узел». 23 декабря 2025 года он описал этапы: «первый шаг — добавить предупреждение, в том числе для Shadowsocks, VMess и insecure; второй шаг — блокировать незашифрованный трафик в публичной сети, обойти смогут только конфигурации, которые нельзя расшарить ссылкой».
>
...
Короткий ответ: **само по себе — нет**, и это позиция автора, а не осторожная оценка со стороны.
...
> Включать имеет смысл там, где есть **посредник или отсутствует внешний TLS**: связки через CDN, транзит через чужой узел, каскады, конфигурации с `security: none`. Для классической схемы `TCP + REALITY + Vision` напрямую с клиента на свой сервер VLESS Encryption не обязателен: конфиденциальность там уже обеспечена, а прибавка — эшелонированная постквантовая защита ценой совместимости и, что менее очевидно, ценой ядерного splice, который на этой схеме перестанет включаться.
>
...
- 🔗 [PR #5067 «VLESS Post-Quantum Encryption»](https://github.com/XTLS/Xray-core/pull/5067) — первоисточник, описание автора
