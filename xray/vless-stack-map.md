...
| Транспорт | `type` / `network` | Во что упакован поток | `tcp` (RAW), `xhttp`, `ws`, `grpc`, `kcp`, `httpupgrade` |
...
**Что проверил автор темы.** В списке «уже проверено и **не** помогает» первым пунктом стоит именно «включение и выключение flow (`xtls-rprx-vision`)». А среди рабочих обходов перечислены: сменить порт с 443 на любой другой; **чтобы остаться на 443 — убрать flow и включить mux**; XHTTP (или старые H2/H3), тоже вместе с mux; пускать через прокси только нужные адреса; наконец, любой не-TLS протокол, который ещё не блокируют. В обновлении от 15 ноября автор формулирует механизм прямо: провайдер применяет шейпинг при достижении некоторого числа одновременных соединений с одним адресом, поэтому «должны работать и любые другие приёмы, уменьшающие количество TLS-соединений».
...
> Есть вторая, независимая схема фильтрации, описанная в [net4people/bbs #490](https://github.com/net4people/bbs/issues/490): при подозрительной подсети сервера соединение «замораживают» после того, как в нём прошло определённое количество пакетов. В обновлении от 17 мая 2026 автор уточнил механику: считаются **не байты, а пакеты — обычно 25 в каждую сторону**, что в среднем даёт около 16 КБ полезной нагрузки; правило переименовали из «tcp 16-20» в «l4-25». Доказательство прямое: тестовый пробник шлёт всего 64 байта, разбитые на 32 отдельных пакета, и ловит заморозку. Бюджет привязан к соединению и по времени не восстанавливается — помогает только новое соединение.
>
> Отсюда следует неприятное: **два правила требуют противоположного**. «Сибирское» правило (#546) наказывает за много соединений и лечится их сокращением, то есть mux. Правило l4-25 наказывает за много пакетов в одном соединении, и его штатный обход — наоборот, размазать трафик по многим коротким соединениям. Мультиплексирование по определению максимизирует пакеты на соединение: восемь потоков делят один 25-пакетный бюджет и умирают разом. Прямых замеров «mux против l4-25» в треде нет — в 79 комментариях #490 мультиплексирование не упоминается вообще, — так что это вывод из механики правила, а не измерение. Но проверять, какое из двух правил действует именно у вас, нужно **до** того, как раздавать профиль: они лечатся взаимоисключающими способами.
...
> Разбирая ту же ноябрьскую волну, RPRX 21 ноября 2025 года предположил, что оператор мог нацелиться именно на параметры набивки в коде Vision — те самые пороги `900 500 900 256`, не менявшиеся три года. Он сразу назвал это догадкой («поэтому я так и предположил»), исходя из того, что помог mux. Через два дня добавил, что неясно даже принципиальное — обучен ли фильтр как чёрный список по трафику Vision или как белый список по браузерному. А ещё через день склонился к тому, что дело вообще не в признаках отдельного соединения: по сообщению одного из участников, под фильтр попадал и настоящий Chrome, то есть считают только соединения. Правда, тут же выяснилось, что и это объяснение шаткое — другой разработчик возразил, что браузер сам ограничен шестью соединениями на домен, поэтому «кратные шести» цифры в наблюдении могут быть артефактом самого Chrome, и нужен отдельный тест.
>
> Практическим следствием всё равно стала настройка `testseed`, позволяющая менять эти пороги ([PR #5270](https://github.com/XTLS/Xray-core/pull/5270), влит 1 декабря 2025), но публичных отчётов «поменял параметры — блокировка ушла» с тех пор не появилось. Подробности — в [[xray/xtls-vision|заметке про Vision]]. Пока это непроверенное предположение, от которого отказался и сам автор гипотезы.
...
>
> Под нож попали Shadowrocket, Hiddify, старые сборки и, что особенно неприятно, **mihomo — навсегда**: он жёстко сообщает версию клиента 1.8.2 и менять это не собирается, так что чинить приходится на сервере. Мотив разработчиков не криптографический, а антицензурный: старое ядро означает устаревший отпечаток uTLS, который сам повышает риск блокировки IP сервера. Порог выбран не по границе уязвимости, а по размеру установочной базы.
>
...
| XHTTP / WS / gRPC | TLS или REALITY | `none` | пусто | ✅ | Проход через CDN, Vision недоступен |
| XHTTP / WS / gRPC | TLS или REALITY | `none` | `xtls-rprx-vision` | ❌ | Ошибка: «XTLS only supports TLS and REALITY directly for now» |
| XHTTP / WS / gRPC | TLS или REALITY | VLESS Encryption | `xtls-rprx-vision` | ✅ | Vision поверх CDN-транспорта; splice недоступен |
...
> Наоборот. VLESS Encryption не меняет внешний вид соединения, а его автор прямо пишет, что для прохода через файрвол нужны REALITY, XHTTP и Vision. Более того, при `security: none` соединение выглядит как поток случайных байтов — а именно такой трафик китайский GFW детектирует энтропийным классификатором.
