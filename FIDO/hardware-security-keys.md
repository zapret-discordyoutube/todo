...
Аппаратный ключ подключается к компьютеру или телефону и выполняет криптографические операции внутри корпуса. Наиболее распространённые способы связи — разъём USB, бесконтактный обмен NFC и радиоканал Bluetooth; названия `USB`, `NFC` и `Bluetooth` обозначают конкретные интерфейсы, а не отдельные протоколы входа. Сокращение `BLE` означает экономичный режим Bluetooth Low Energy, который используют некоторые беспроводные сценарии.

Устройство, которое хранит закрытый ключ и подписывает запрос сайта, в стандартах называют `authenticator` («аутентификатор»). Внешний брелок называют `roaming authenticator` («переносной аутентификатор»), а встроенное в телефон или компьютер хранилище — `platform authenticator` («платформенный аутентификатор»).

Криптографическая пара состоит из закрытого и открытого ключей. Закрытый ключ создаёт подпись и остаётся у пользователя, а открытый ключ хранится на сервере и только проверяет подпись. Поэтому утечка серверной копии не позволяет подписать новый запрос.

Ключ должен доказать, что запрос пришёл от нужного сайта, и подписать одноразовый случайный запрос сервера. Такой запрос называют `challenge` («вызов»), адрес страницы — `origin` («происхождение запроса»), а разрешённую область сайта — `RP ID` («идентификатор сервиса»). RP происходит от relying party и означает сайт, который полагается на результат проверки.

Подписанный результат входа называют `assertion` («утверждение аутентификатора»). Всю схему, в которой сервер выдаёт случайный challenge и проверяет связанный с ним ответ, называют challenge-response («вызов — ответ»).

На одном ключе можно хранить несколько записей с закрытыми ключами и сведениями об аккаунтах. Запись, которую устройство умеет найти и предложить без заранее присланного идентификатора, называют `discoverable credential` («обнаруживаемые учётные данные»), а её внутреннюю структуру — `credential source` («источник учётных данных»).

Если устройство не умеет само найти запись и сервер обязан заранее вернуть её идентификатор, используют выражение `non-discoverable credential` («необнаруживаемые учётные данные»). Этот вариант обычно расходует меньше внутренней памяти устройства.

Программа внутри ключа принимает команды, управляет памятью и выполняет криптографию. Эту программу называют `firmware` («прошивка»); закрытая прошивка доступна только производителю, а `open-source firmware` («прошивка с открытым исходным кодом») доступна для независимого изучения.

Некоторые модели содержат отдельную микросхему, которая изолирует ключевой материал и выполняет защищённые операции. Такую микросхему называют `secure element` («защищённый элемент»); её наличие не следует автоматически из одной только совместимости с FIDO.

Отраслевой альянс FIDO (Fast IDentity Online) выпускает открытые правила криптографического входа, по которым работают сайты, браузеры и устройства. Внешний ключ общается с браузером через CTAP (Client to Authenticator Protocol, «протокол связи клиента с аутентификатором»), а сайт использует веб-интерфейс WebAuthn (Web Authentication, «веб-аутентификация»). Современную связку WebAuthn и CTAP называют FIDO2, а старый режим второго фактора — U2F (Universal 2nd Factor, «универсальный второй фактор»).

Помимо входа на сайты, некоторые ключи хранят секреты для одноразовых паролей, сертификаты или ключи электронной подписи. Одноразовый пароль называют OTP (One-Time Password), его вариант по времени — TOTP (Time-based OTP), а вариант по счётчику — HOTP (HMAC-based OTP); открытая инициатива их совместимости называется OATH. Корпоративные удостоверения личности на смарт-картах описывает PIV (Personal Identity Verification), ключи электронной почты и подписи — OpenPGP, а серверное защищённое хранилище ключей — HSM (Hardware Security Module, «аппаратный модуль безопасности»).

OpenPGP часто используют через программу GPG для шифрования, подписи и удалённого входа. Протокол удалённого защищённого доступа называется SSH, а программа `gpg-agent` позволяет применять OpenPGP-ключ в таком подключении. Защищённое соединение сайта или приложения использует TLS; клиентский TLS-сертификат позволяет серверу проверить не только себя перед пользователем, но и пользователя или устройство перед сервером.

Сайт иногда просит устройство доказать происхождение и свойства модели. Такое доказательство называют `attestation` («аттестация»), а независимую проверку заявленной модели и уровня защиты — `FIDO certification` («сертификация FIDO»). В каталоге уровень дополнительных проверок обозначают `Authenticator Security Level`; значения L1, L2, L3 и L3+ описывают последовательно более строгие требования, но относятся только к указанной модели и версии.

Локальное касание сообщает, что человек физически взаимодействовал с ключом, а персональный цифровой код PIN или отпечаток подтверждают разблокировку владельцем. Первый признак называют `user presence` (UP, «присутствие пользователя»), второй — `user verification` (UV, «проверка пользователя»).

Ключ может хранить запись только у себя или разрешать её резервное копирование и использование на других устройствах. В первом случае используют выражение `single-device credential`, во втором — `multi-device credential`; пользовательское название таких ключей доступа — `passkey`.

Вход с телефона на чужом компьютере требует QR-кода, проверки близости по Bluetooth и защищённого канала между устройствами. Такой способ связи называют `hybrid transport` («гибридный транспорт»).

Для регулируемых отраслей производитель может пройти отдельную проверку криптографического модуля. Обозначение `FIPS 140-3` относится к американскому стандарту сертификации криптографических модулей, а `Sunset List` — к списку старых сертификатов, которые перестают считаться действующими для новых закупок.

Если основной ключ потерян, сервис может разрешить запасной способ входа или восстановления. Английские термины `fallback` и `recovery` означают такой запасной путь и процедуру возврата доступа, а `recovery code` — заранее сохранённый одноразовый код восстановления.

Набор программных функций для обращения к устройству называют API (Application Programming Interface, «программный интерфейс приложения»). Серверные криптографические модули часто предоставляют стандартный интерфейс PKCS#11, через который приложение просит создать ключ или подпись, не извлекая закрытый материал.
