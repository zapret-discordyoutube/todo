...
## Что собрано в папке FIDO

Эта папка посвящена способам входа, при которых устройство доказывает серверу владение закрытым криптографическим ключом. Читатель начинает с привычной проблемы паролей и постепенно доходит до регистрации ключа, проверки подписи, защиты от поддельных сайтов и восстановления доступа после потери устройства. Материалы написаны так, чтобы первый разбор можно было читать без подготовки по криптографии.

Две обзорные статьи создают основу раздела. [[FIDO/fido-history|История FIDO]] объясняет, зачем понадобились общие стандарты и как от отдельного второго фактора отрасль пришла к беспарольному входу. [[FIDO/fido-protocols|Общая механика протоколов]] следует за одним запросом от сервера к браузеру, операционной системе и аутентификатору, а затем разбирает проверки на обратном пути.

Следующий слой нужен тем, кто хочет понять границы ответственности. [[FIDO/webauthn|Браузерная часть]] показывает, какие данные формирует страница и что обязан проверить сервер. [[FIDO/ctap|Связь с внешним ключом]] описывает команды устройства, проводные и беспроводные каналы, локальный PIN и защиту от перебора. Отдельные статьи об [[FIDO/u2f|универсальном втором факторе]] и [[FIDO/uaf|раннем беспарольном входе]] сохраняют исторический контекст и объясняют совместимость со старыми реализациями.

Практическая часть раздела помогает выбрать способ хранения ключей. [[FIDO/passkeys|Ключи доступа]] разбирают синхронизацию между устройствами, вход телефоном на чужом компьютере и зависимость от восстановления аккаунта провайдера. [[FIDO/hardware-security-keys|Физические ключи безопасности]] сравнивают возможности устройств, дополнительные протоколы, открытые прошивки, сертификацию и правила резервирования. Эти две заметки отвечают на разные вопросы: первая описывает пользовательский сценарий, вторая помогает подобрать и безопасно использовать отдельное устройство.

Раздел не требует чтения от первой до последней страницы. Для понимания принципа достаточно истории и общей механики; после них можно перейти прямо к ключам доступа или аппаратным ключам. Разработчику полезнее продолжить браузерной частью и протоколом внешнего устройства, а старые стандарты читать тогда, когда нужна совместимость или исторический контекст.
