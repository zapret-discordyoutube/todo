...
> [!info] О чём заметка
> FIDO — это и промышленный альянс, и семейство стандартов сильной онлайн-аутентификации: U2F, UAF, FIDO2/WebAuthn и passkeys. Здесь — что это такое, на каких принципах построено и как за десятилетие путь дошёл от идеи двух инженеров до входа по ключу доступа во многих массовых сервисах. Механика самих протоколов — в [[FIDO/fido-protocols|заметке о протоколах FIDO]], практика выбора железа — в [[FIDO/hardware-security-keys|статье о физических ключах]].
...
- Три поколения стандартов: **U2F** и **UAF** из FIDO 1.0 → **FIDO2** как связка W3C **WebAuthn** и FIDO **CTAP** → **passkeys**, то есть discoverable credentials FIDO2 в single-device или синхронизируемом варианте.
- 5 мая 2022 года Apple, Google и Microsoft объявили планы расширить поддержку passkeys. iOS 16 получила их в сентябре 2022 года, Chrome Stable — в декабре 2022 года, а встроенный интерфейс Windows 11 начал распространяться в сентябре 2023 года.
...
Альянс появился как ответ на проблему общего секрета. Пароль вводит пользователь, а сервер хранит его проверочное представление. Атакующий может выманить пароль фишингом, подобрать слабый вариант, найти его в чужой утечке или атаковать процедуру восстановления. Дополнительный SMS-код меняет число шагов, но остаётся переносимым секретом: жертва способна отдать его поддельному сайту.
...
Касание и проверка владельца означают разные вещи. User presence (UP) подтверждает физическое действие: например, касание кнопки. User verification (UV) означает локальную проверку PIN-кодом или биометрией. FIDO2 поддерживает оба режима, а сервис выбирает политику `required`, `preferred` или `discouraged`; PIN или биометрия не обязательны для каждой операции FIDO2.
...
По официальному изложению FIDO, история началась в 2009 году, когда Рамеш Кесанупалли из Validity Sensors предложил Мишелю Барретту, тогдашнему CISO PayPal, использовать биометрию для входа в PayPal. Барретт настаивал на открытом стандарте, который не зависел бы от одного производителя. Альянс сформировали в июле 2012 года, а 12 февраля 2013 года о нём объявили публично. Роли первых участников различались: Lenovo, Nok Nok Labs, PayPal и Validity Sensors вошли как founding board members, Agnitio как founder associate, Infineon как founder sponsor.
...
U2F работал только как второй фактор и использовал отдельный браузерный API. 12 ноября 2015 года Microsoft, Google, PayPal и Nok Nok Labs подали FIDO 2.0 Platform Specifications как W3C Member Submission. Рабочая группа Web Authentication начала работу 17 февраля 2016 года и создала **[[FIDO/webauthn|WebAuthn]]**, стандартный JavaScript API для регистрации и входа. FIDO Alliance параллельно развивал **[[FIDO/ctap|CTAP2]]**, протокол клиента с внешним аутентификатором; U2F получил имя CTAP1. Связка WebAuthn и CTAP стала FIDO2.
...
Поддержка вышла поэтапно. iOS 16 стала доступна 13 сентября 2022 года. Android и Chrome начали публичное тестирование passkeys в октябре, а Chrome Stable M108 получил поддержку 8 декабря. Google Accounts добавили passkeys 3 мая 2023 года и сделали их вариантом по умолчанию для личных аккаунтов 10 октября. Встроенный интерфейс passkeys в Windows 11 начал распространяться 26 сентября 2023 года.
...
| Год | Событие |
|---|---|
| 2009 | Разговор Validity Sensors и CISO PayPal об открытом стандарте (по официальной истории FIDO) |
