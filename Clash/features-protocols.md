...
> [!info] О чём заметка
> Разбор актуальных возможностей ядра **mihomo** (бывший Clash.Meta) — какие протоколы оно поддерживает как клиент и как сервер, чем маскирует трафик, как устроены TUN, DNS-подсистема, сниффер и наборы правил. Что такое mihomo и откуда он взялся — в заметке [[Clash/mihomo|mihomo (Clash.Meta)]]; базовое устройство конфига Clash (порты, группы, правила, fake-ip) разобрано в [[Clash/clash-core|Ядре Clash]] и здесь не повторяется.
...
|---|---|
| Shadowsocks / SSR | Классика экосистемы; современные AEAD-шифры, плагины обфускации, в свежих версиях — обфускация `jls` |
| VMess | Наследие v2ray; поддерживает транспорты WS/gRPC/HTTP2 и мультиплекс, в релизах 2026 добавлены `mkcp` и `tlsmirror` |
...
| Trojan | Маскировка под обычный HTTPS; сочетается с WS/gRPC и обфускациями |
...
| TUIC (v4/v5) | Ещё один QUIC-протокол с быстрым установлением UDP-сессий |
| WireGuard | Полноценный WG-клиент прямо в ядре — можно использовать как обычный outbound наравне с прокси |
| ShadowTLS | Обёртка, прячущая произвольный протокол за настоящим TLS-рукопожатием с чужим доменом |
...
| AnyTLS | Относительно новый протокол с акцентом на маскировку под TLS; в mihomo поддерживается и как outbound, и как listener |
...
Базовые типы правил (`DOMAIN`, `DOMAIN-SUFFIX`, `IP-CIDR`, `GEOIP`, `PROCESS-NAME`, `MATCH`) описаны в [[Clash/clash-core|заметке про ядро Clash]]. mihomo добавил к ним ощутимо больше выразительности:
...
DNS в mihomo — самостоятельная подсистема, а не одна строка с адресом сервера. Поверх режимов `fake-ip` и `redir-host` (разобраны в [[Clash/clash-core|базовой заметке]]) добавлены:
...
Управляющий HTTP-API (см. раздел про `external-controller` в [[Clash/clash-core|ядре Clash]]) в mihomo расширен: добавлены ручки статистики памяти, отладочные эндпоинты, управление провайдерами правил и узлов. Официальный дашборд — **metacubexd**; из популярных сторонних — zashboard и наследники yacd. Всё это одинаково применимо и к ядру внутри графического клиента: большинство оболочек просто открывают тот же API у своего встроенного mihomo.

## 📚 См. также

- [[Clash/mihomo|mihomo (Clash.Meta)]] — история проекта, состояние и совместимость.
- [[Clash/clash-core|Ядро Clash]] — базовое устройство конфига, правила, группы, fake-ip, Clash API.
- [[Clash/clients|Клиенты на ядре Clash/mihomo]] — где всё это включается кнопками.
- [[Clash/vs-sing-box|mihomo против sing-box и Xray]] — как тот же набор функций устроен у соседей.
...
> При желании вы можете натренировать ИИ на наших статьях. Исходное форматирование и скачивание всего репозитория одним zip-архивом доступны на GitHub: [исходник этой заметки](https://github.com/youtubediscord/todo/blob/main/Clash/features-protocols.md) · [весь репозиторий](https://github.com/youtubediscord/todo/tree/main).
