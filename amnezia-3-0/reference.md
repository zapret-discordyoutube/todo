...
### «Не работает на операторе X» — три разные причины

Жалобы формулируются одинаково, а лечатся по-разному. Два полевых наблюдения мейнтейнера установщика [amneziawg-installer](https://github.com/bivlked/amneziawg-installer) — оба по второму поколению, по третьему сопоставимых отчётов пока нет.

**Порт.** 26 июня 2026 года на МТС туннель не поднимался на порту 39743/UDP и встал сразу после переноса на 443/UDP — тот же конфиг, тот же сервер. Контроль: на трёх домашних провайдерах в Москве этот же сервер работал и на исходном порту, а потери в туннеле не отличались от обычного UDP до того же адреса. Причина не выяснялась, проверялся только сам факт. Тогда же похожее сообщал ещё один пользователь МТС, у которого не помогал и пресет для мобильных сетей.

**Адрес.** 6 августа 2026 года у другого пользователя, тоже на сотовой сети, порт остался прежним, а помогла смена IP-адреса у хостера: после замены адреса на сервере и в клиентах заработало и на сотовой, и дома. VPS того же хостера в той же стране поднимался из коробки, так что «страной» это не объяснялось. Ключевая деталь: вместе с туннелем не отвечали SSH и пинг до того же адреса — значит, дело было в самом адресе или маршруте до него, а не в AmneziaWG.

Отсюда порядок диагностики, который экономит часы: сначала проверить доступность адреса (SSH, пинг, желательно из другой сети), затем попробовать порт 443/UDP, и только потом трогать обфускацию. Если недоступен весь адрес, никакая конфигурация не поможет — это тот самый случай «блокировки по адресу», для которого смена версии протокола бесполезна.
