...
> [!tip] Для IP-адресов отдельный профиль обычно не нужен
> Если исключать нужно не домен, а IP-адрес или подсеть, в стоковых пресетах уже есть готовый профиль «Исключения (RU сайты)» со стратегией `pass` — адрес достаточно дописать в его [[ipset|ипсет]]. Это быстрый путь «в общую кучу»; вариант «по красоте» — отдельный `pass`-профиль на каждый ресурс. Создавать новый профиль с нуля имеет смысл прежде всего для **доменных** исключений, как в примере ниже.
