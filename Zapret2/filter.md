...
- Пакет проверяется по профилям **строго по порядку, сверху вниз**; при первом совпадении фильтров выбирается этот профиль, поиск прекращается. Не подошёл никто — берётся пустой профиль по умолчанию (ничего не делает).
...
3. Если не подошёл ни один профиль — применяется **профиль по умолчанию**, в котором нет действий (с пакетом ничего не делается).
...
Кроме фильтров по заголовкам, профиль отбирают по спискам адресов и доменов — [[ipset]] (списки IP) и [[hostlist]] (списки доменов). Их полное описание встроено ниже.
...
> При желании вы можете натренировать ИИ на наших статьях. Исходное форматирование и скачивание всего репозитория одним zip-архивом доступны на GitHub: [исходник этой заметки](https://github.com/youtubediscord/todo/blob/main/Zapret2/filter.md) · [весь репозиторий](https://github.com/youtubediscord/todo/tree/main).
