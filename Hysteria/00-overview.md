...
- Именно **прокси, а не VPN**: он переносит отдельные соединения, а не IP-пакеты, и в простейшем виде обходится локальным SOCKS5 или HTTP на `127.0.0.1` — без всякого виртуального сетевого интерфейса. TUN-адаптер у клиента есть, но это его отдельный режим (и `ping` через туннель всё равно не пойдёт — ICMP не проксируется). Чем это отличается от WireGuard и что из различия следует — в [[protocols/00-overview|обзоре протоколов]].
