---
description: "Почему антивирусы ругаются на Zapret и WinDivert, 8 правил проверки сборки, сверка хэшей winws.exe и разбор десяти реальных вирусных подделок."
---

> [!mirror] Резервное зеркало
> Актуальная версия этой страницы — на основной вики: [wiki.zapret.moe/virus/virus](https://wiki.zapret.moe/virus/virus)
...
**1. В папке `bin` должен быть один `.exe` — `winws.exe`.** Видишь второй, как бы он ни назывался (`StartWin.exe`, `Windiver.exe`, «Быстрый запуск») — выбрасывай всю сборку. Второй файл никто не кладёт «для удобства».
...
**4. Смотри размер `.bat`-файлов.** Обычный — 2–4 килобайта. Если батник весит мегабайты, в нём спрятана программа. Открой блокнотом: увидишь стену из букв и цифр и слово `certutil` — это вирус.
...
**6. Антивирус молчит — это не значит «чисто».** Один и тот же файл: сам по себе его ловят 51 антивирус из 71, а внутри большого архива — только 10 из 58. Поэтому проверяй на [VirusTotal](https://www.virustotal.com) **распакованный** файл, а не архив.
...
> [!tip] Что делать вместо проверок
> Не искать сборку через поиск, рекламу и видео на YouTube. Брать по ссылкам из [[Zapret/download|официальных источников]] — там же лежат хэши. Всё, что нашлось само (красивый сайт, «новая версия 4.0», «фикс 2026») — считай чужим, пока не доказано обратное.
...
Впервые вирусы под предлогом Zapret начал распространять телеграм-канал peekbot: https://github.com/Flowseal/zapret-discord-youtube/issues/794. Будьте внимательны! Также у них имеется вредоносный сайт https://gitrok.com!
...
Разбор цепочки целиком, включая приём восстановления скрытого репозитория по числу из ссылки: [[hidemydiscord-loader-august-2026|HideMyDiscord — рабочий обход блокировок с загрузчиком в комплекте]].
