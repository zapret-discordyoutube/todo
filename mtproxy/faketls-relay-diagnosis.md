...
- **Размер первого пакета.** Долго был главным подозреваемым: проба шлёт 517 байт — один TCP-сегмент, клиент 1718–1823 — гарантированно два. Проверено прямым сравнением: тридцать пар структурно идентичных hello (517 и 1800 байт, все declared lengths и padding-расширение выращены согласованно, так что меняется только длина) с проблемной машины. **Все шестьдесят получили `ServerHello` с верным digest**, времена одинаковые (~60 мс, единственный выброс 309 мс на 1800 байт). Второй сегмент релей дожидается без нареканий.

Тем самым исключено всё, что находится **на проводе**: сеть, IP, количество и длительность соединений, форма и размер пакета. Что бы ни ломалось, оно ломается внутри процесса клиента — между «hello построен» и «hello покинул машину».

Рабочая гипотеза после этого: **hello оставался в очереди сокета**. `QTcpSocket::write()` не отправляет данные, а ставит их в буфер; уходят они, когда поток в следующий раз вернётся в event loop. Клиент же сразу после записи запускает пятисекундный отсчёт ожидания ответа. Картина в логе получается ровно наблюдаемая: `ch_accepted == ch_bytes` (сокет байты принял), `rx_after_ch=0`, `close_origin=local_timeout`. И объясняется, почему первые соединения проходят: поток ещё свободен, а дальше он занят другими сокетами и созданием ключей.

В коде асимметрия была прямо видна: фрагментированный путь записи всегда делал `flush()` после первой части, а нефрагментированный — тот, которым идёт **каждое** соединение к mtproxy, поскольку фрагментация для него выключена, — не делал ни разу; хвостовая запись тоже. Исправлено в `6a059504db` (флаш в обоих путях).

Оговорка на будущее: это **не доказанная** причина, а единственное найденное отличие клиента от пробы, дающее в точности такую форму отказа. Решающая проверка дешёвая — запустить пробу **в тот момент, когда клиент висит**: если она отвечает, вина точно внутри клиента.
