...
> [!warning] У версии «дело в теле GREASE» осталась одна дыра
> Опустошение тела укорачивает hello на байт, то есть **тело и длина менялись вместе**, и прогон выше не различает «отвергают непустое тело GREASE» и «отвергают hello длиной 1718». Развязка требует одинакового размера при разном теле: длина компенсируется внутри GREASE key_share, у которого она произвольна, так что запись, рукопожатие, список расширений и итоговый размер совпадают с базовой рукой до байта.
>
> Тому же опыту заодно поручены остальные незакрытые формулировки: тело любой длины или ровно один байт; важна ли позиция несущего расширения — это заодно отделяет «любое GREASE-расширение с телом» от «последнее расширение hello непустое», что подходит к данным не хуже; важно ли значение байта; тело или само наличие GREASE-расширений; и держится ли правило на чужих байтах (дамп браузера с опустошённым GREASE). Проба — `test8.py`, тринадцать рук, калибровка размеров печатается и проверяется до прогона.
>
