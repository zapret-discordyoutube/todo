...
Тем самым исключено всё, что находится **на проводе** для *одиночного* рукопожатия: сеть, IP, количество и длительность соединений, форма и размер пакета.
...
Решающая проверка, которая ещё не сделана: запустить пробу **в тот момент, когда клиент висит**, на том же адресе. Проба отвечает → блокируется именно клиентский процесс/поток; проба замолчала так же → релей или маршрут, а клиент ни при чём.
