...
> [!warning] Механизм не найден
> Известно, **какой** отпечаток отвергают, и этого достаточно, чтобы починить клиент. Неизвестно, **по какому признаку** его отличают. Пять названных версий закрыты, JA4 в том числе, а побайтовое сравнение двух шаблонов оставляет ровно одно измеренное различие: у `chrome_modern` одно GREASE-расширение несёт один нулевой байт, у `yandex` оба пустые. Всё прочее либо идентично, либо это сами GREASE-значения, случайные в каждом hello. При этом у настоящего браузера этот байт **есть**, так что версия «дело в байте» сама себе противоречит, пока не проверена на живой сети.
>
> Гарантированный способ дойти до конца — деление пополам: шаблоны побайтово сравнимы, различающихся полей единицы, и можно собирать гибриды, перенося поля по одному, пока не останется единственное. Пока это не сделано, причина в заметке стоит как «отпечаток такой-то отвергается», а не как объяснение.
...
- Диагностика: при «hello ушёл, ответа нет» в лог пишутся состояние сокета, число непрочитанных байт и число уведомлений о чтении (`fb0c74e0d4`) — это отличает «ответ не пришёл» от «ответ пришёл и не был прочитан».
