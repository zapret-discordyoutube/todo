...
> [!note] Обновление 8 августа 2026: две правки по результатам измерений
> Разделы [[#Правило первого кадра]] и [[#Мифа про «только DC2 и DC4» не существует]] опираются не на чтение кода, а на живые соединения с релеями (около 350 попыток в трёх независимых сериях). Они исправляют два утверждения, которые до этого кочевали по всем источникам, включая раннюю версию этой заметки:
> - «один MTProto-пакет — одно WebSocket-сообщение» **не является требованием сервера**; значение имеет только длина первого кадра;
...
4. Единственное требование релея к нарезке — **первый бинарный кадр после апгрейда должен содержать не меньше 64 байт**, то есть весь init целиком. Всё остальное можно резать как угодно: границы MTProto-пакетов релею безразличны. Раньше в этой заметке утверждалось обратное — см. [[#Правило первого кадра]].
...
Все четыре проекта отправляют один MTProto-пакет одним WebSocket-сообщением, а 64-байтный init — отдельным сообщением перед ним. Долгое время это считалось обязательным: мол, иначе границы кадров перестанут совпадать с границами пакетов и трафик перестанет походить на браузерный Telegram Web. **Это не так** — см. следующий раздел.

---

## Правило первого кадра

> [!tip] Единственное, что релей действительно требует
> Первый бинарный WebSocket-кадр после апгрейда обязан содержать **не менее 64 байт** — весь obfuscated2-заголовок. Всё, что отправляется дальше, можно резать на кадры произвольно: границы MTProto-пакетов релею безразличны.
...
**Практический вывод: непрерывный байтовый поток по WSS полностью законен.** Транспорт может писать в сокет как угодно — склеивать пакеты, резать их пополам, отдавать по мере готовности сокета. Единственное исключение — самая первая запись после апгрейда. Если ваш транспорт режет исходящий буфер по фиксированному размеру и первый кусок случайно окажется меньше 64 байт, соединение умрёт без единого сообщения об ошибке. Надёжнее всего гарантировать это в самом транспорте: придерживать начальные байты, пока их не наберётся 64, и только потом выпускать первый кадр.
...
**2. Дальше — обычный байтовый поток.** Не заводите отдельную очередь сообщений и не выравнивайте кадры по границам MTProto-пакетов: это лишняя сложность, лишние копирования и лишний источник ошибок, а сервер разницы не видит. Пишите так же, как в TCP-транспорт, и режьте по любому удобному размеру.
