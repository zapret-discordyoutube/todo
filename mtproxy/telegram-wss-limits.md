...
8. Клиентские форки долго **не откатывались на обычный TCP**: при недоступном релее попытки повторялись, пока пользователь не выключит транспорт руками, а причина отказа в интерфейс не выводится. В ZaStoGram для Android откат добавлен в версии 1.1.18 — датацентр с закрытым релеем уходит на прямое соединение.
